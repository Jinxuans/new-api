...
func ChargeViolationFeeIfNeeded(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, apiErr *types.NewAPIError) bool {
	...
	// The violation belongs to the request that already crossed TokenAuth.
	// A refund hold installed while that request was in flight must stop later
	// requests, but must not let this already-incurred fee disappear.
	if _, err := postConsumeQuotaWithResult(relayInfo, feeQuota, 0, true, true); err != nil {
		logger.LogError(ctx, fmt.Sprintf("failed to charge violation fee: %s", err.Error()))
		return false
	}

	model.UpdateUserUsedQuota(relayInfo.UserId, feeQuota)
